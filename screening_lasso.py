...
def compute_duality_gap_prox_lasso(X, y, w, lbd, w_0, alpha=1e100000000000):
        ...
        if not isspmatrix(X):
            #print(alpha)
            #print(np.sum(v**2))
            #print(alpha / 2 * np.sum(v**2))
            #print("v", v)
            #print(np.sum(v**2))
            #print(" alpha / 2 * np.sum(v**2)",  alpha / 2 * np.sum(v**2))
            dualcost = - 0.5 * np.sum(nu * nu) + np.sum(nu * y)
                # v.T.dot(w_0) - \
            #print("dualcost", dualcost)
        else:
            #print(alpha / 2 * np.sum(v**2))
            dualcost = -0.5 * spnorm(nu)**2 + (nu.multiply(y)).sum()
                # v.T.dot(w_0)
...
def weighted_prox_lasso_bcd_screening(X, y, lbd, w_0=np.array([]),alpha=1e1000000, nbitermax=10000,
                                      winit=np.array([]),
